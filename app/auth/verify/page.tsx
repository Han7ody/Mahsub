...
function maskDestination(value: string | null, via: string | null) {
  ...
  if (via === "email" || value.includes("@")) {
    ...
    const maskedDomain =
      domainName.length <= 1 ? `${domainName}***` : `${domainName[0]}***${domainName.slice(-1)}`;
...
function VerifyPageContent() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Registration completion: create business + owner membership (same as D)
      if (!isLoginFlow) {
        ...
        if (pendingData) {
          ...
          if (business?.id) {
            ...
            if (memberError) {
              // Non-blocking if RLS blocks it; owner can still access via businesses.owner_user_id
              // Keep silent but log to console for dev.
              console.warn("Membership insert blocked by RLS:", memberError);
            }
          }
  ...
  const infoText = via === "email" ? "تم إرسال رمز التحقق إلى بريدك الإلكتروني" : "تم إرسال رمز التحقق";
  ...
  const handleResend = () => {
    ...
    const resend = async () => {
      ...
      // Demo mode: just reset timer
      if (!USE_BACKEND) {
        showToast("تم إعادة الإرسال (وضع تجريبي)", "success");
        return;
      }
                ...
                <div className="flex flex-col items-center gap-2 text-sm text-text-muted dark:text-text-muted-dark">
                  <div className="flex items-center gap-2 font-medium">
                    ...
                    <span className="text-primary font-bold tabular-nums">
                      {secondsLeft > 0 ? `00:${secondsLeft.toString().padStart(2, "0")}` : "جاهز"}
                    </span>
                  </div>
